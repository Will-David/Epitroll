...
# Check if running as root (only needed for some installation steps, but aliases are per-user)
# However, if we want to run 'sudo punish' without password, the script itself is system-wide.
# We will target the current user's config files.
